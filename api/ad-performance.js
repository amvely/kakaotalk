...
function decodeTextBuffer(buf){
  ...
  // 네이버 대용량보고서 다운로드 URL이 계정/환경에 따라 zip으로 내려오는 경우가 있어 zip 1개 파일을 직접 풉니다.
  if(b.length >= 4 && b[0] === 0x50 && b[1] === 0x4b && b[2] === 0x03 && b[3] === 0x04){
    let off = 0;
    while(off + 30 <= b.length && b[off] === 0x50 && b[off+1] === 0x4b && b[off+2] === 0x03 && b[off+3] === 0x04){
      const method = b.readUInt16LE(off + 8);
      const compSize = b.readUInt32LE(off + 18);
      const nameLen = b.readUInt16LE(off + 26);
      const extraLen = b.readUInt16LE(off + 28);
      const dataStart = off + 30 + nameLen + extraLen;
      const dataEnd = dataStart + compSize;
      const name = b.slice(off + 30, off + 30 + nameLen).toString('utf8');
      if(dataEnd > b.length) break;
      const part = b.slice(dataStart, dataEnd);
      let out;
      if(method === 0) out = part;
      else if(method === 8) out = zlib.inflateRawSync(part);
      else throw new Error(`지원하지 않는 StatReport zip 압축 방식입니다(method=${method}, file=${name})`);
      if(!/\/$/.test(name)) return out.toString('utf8').replace(/^﻿/, '');
      off = dataEnd;
    }
  }
  // gzip으로 내려오는 경우도 방어합니다.
  if(b.length >= 2 && b[0] === 0x1f && b[1] === 0x8b) return zlib.gunzipSync(b).toString('utf8').replace(/^﻿/, '');
  return b.toString('utf8').replace(/^﻿/, '');
}
async function responseTextOrDecoded(resp){
...
async function naverRawReq(cid, lic, sec, method, pathWithQuery){
  ...
  const sig = makeNaverSignature(sec, timestamp, m, pathOnly);
  const resp = await fetch(NAVER_BASE + pathWithQuery, { method:m, headers:{ 'X-Timestamp':timestamp, 'X-API-KEY':lic, 'X-Customer':cid, 'X-Signature':sig }});
  const txt = await responseTextOrDecoded(resp);
...
const HEADERLESS_REPORT_SCHEMAS = {
  // 파워링크 검색어 보고서 / StatReport reportTp=EXPKEYWORD
  EXPKEYWORD: ['date','customerId','campaignId','adgroupId','searchTerm','mediaCode','pcMobileType','searchKeywordType','impression','click','cost','viewCount'],
  // 쇼핑검색 검색어 상세 보고서 / StatReport reportTp=SHOPPINGKEYWORD_DETAIL
  SHOPPINGKEYWORD_DETAIL: ['date','customerId','campaignId','adgroupId','searchTerm','adId','businessChannelId','hour','regionCode','mediaCode','pcMobileType','impression','click','cost','sumOfAdRank','viewCount'],
  // 쇼핑검색 검색어 전환 상세 보고서 / StatReport reportTp=SHOPPINGKEYWORD_CONVERSION_DETAIL
  SHOPPINGKEYWORD_CONVERSION_DETAIL: ['date','customerId','campaignId','adgroupId','searchTerm','adId','businessChannelId','hour','regionCode','mediaCode','pcMobileType','conversionMethod','conversionType','conversionCount','salesByConversion']
};
function rowFromSchema(vals, schema, statDt){
  const row={};
  schema.forEach((k,i)=>{ row[k] = vals[i] ?? ''; });
  if(!row.date) row.date = statDt || '';
  return row;
}
function headerlessObject(vals, reportTp, statDt){
  const up=String(reportTp||'').toUpperCase();
  const schema = HEADERLESS_REPORT_SCHEMAS[up];
  // 보고서명이 확정되어 있으면 정의서의 고정 열 순서를 최우선으로 사용합니다.
  // 숫자 컬럼을 휴리스틱으로 추정하면 customerId/hour/mediaCode/sumOfAdRank 때문에 노출·클릭·비용이 밀릴 수 있습니다.
  if(schema && vals.length >= schema.length) return rowFromSchema(vals, schema, statDt);

  // 알 수 없는 변형 보고서용 fallback
  const row={};
  ...
  const textCandidates = vals.slice(afterGroup, Math.max(afterGroup, tailStart))
    .map(v=>String(v||'').trim())
    .filter(v=>v && !isDateCell(v) && !looksLikeId(v,'cmp') && !looksLikeId(v,'grp') && !looksLikeId(v,'ad') && !looksLikeId(v,'nad') && !looksLikeId(v,'bsn') && !looksLikeId(v,'chn') && !isMetricLike(v));
  ...
  if(/CONVERSION/.test(up)){
    ...
    const perf = last.length >= 5 ? last.slice(0,3) : nums.slice(-3);
    row.impression = perf[0]?.v || 0;
    row.click = perf[1]?.v || 0;
    row.cost = perf[2]?.v || 0;
    row.viewCount = last.length >= 5 ? last[4]?.v || 0 : 0;
  }
  return row;
}
...
function pickExact(row, aliases){
  const m = row.__norm || (row.__norm = normMap(row));
  for(const a of aliases){ const nk=normHeader(a); if(m[nk] !== undefined && m[nk] !== '') return m[nk]; }
  return '';
}
...
const H = {
  ...
  conversionType:['conversionType','Conversion Type','전환유형','전환 유형']
};
function normalizeSearchTermStatRow(raw, reportTp, statDate){
  ...
  if(isConv){
    ...
    const convType = toStr(pick(raw,H.conversionType)).toLowerCase();
    const isPurchase = !convType || /purchase|purchasing|구매|^1$/.test(convType);
    base.purchaseCcnt = numCell(pickExact(raw,H.purchaseCcnt)) || (isPurchase ? base.conv : 0);
    base.purchaseConvAmt = numCell(pickExact(raw,H.purchaseConvAmt)) || (isPurchase ? base.revenue : 0);
  }else{
